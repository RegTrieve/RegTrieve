...
def main():
  ...
  #TODO: 使用asr model生成human以及machine updated file, 就跟heysquad一样的json(同时计算wer regression)。然后再用下游模型去计算
    ...
    old_map_file = "%s/%s-map.json" % (predict_dir, opt.oldmodel_path)
    new_file = "%s/%s.json" % (predict_dir, opt.newmodel_path)

    cmp_old_file = "%s/%s-%s-map-cmp.json" % (predict_dir, opt.oldmodel_path, opt.oldmodel_path)
    cmp_new_file = "%s/%s-%s-cmp.json" % (predict_dir, opt.oldmodel_path, opt.newmodel_path)
    ...
    logger.info(f'1. eval the old model without map:{opt.oldmodel_path}')
    # load align info
    ...
    logger.info(f'2. eval the old model with map:{opt.oldmodel_path}')
    # load align info
    align_wrapper = AlignWrapper.load_from_config(opt)
    test_old_map_wers, res_map_old = eval_single_model(
        opt.old_model,
        old_path,
        dev_ref_ids=dev_ref_ids,
        dev_ref_hashmap=dev_ref_hashmap,
    ...
    dev_trans_old_map = deepcopy(dev_data)
    fill_ref_data(dev_trans_old_map, res_map_old)
    with open(old_map_file, 'w') as f:
        json.dump(dev_trans_old_map, f)

    logger.info(f'3. eval the new model without map:{opt.newmodel_path}')
    ...
    logger.info('Compare old vs. old-map')
    cmp_map_old = nfr_validate(test_old_wers, test_old_map_wers)
    with open(cmp_old_file, 'w') as f:
        json.dump(cmp_map_old, f)

    logger.info('Compare old-map vs. new')
    cmp_map_new = nfr_validate(test_old_map_wers, test_new_wers)
    with open(cmp_new_file, 'w') as f:
        json.dump(cmp_map_new, f)
